...
namespace Dynamo.Controllers.Util
{
    ...
    public async Task<IActionResult> UploadDatabase()
    {
        ...
        var boundary = MultipartRequestUtil.GetBoundary(
        ...
        while (section != null)
        {
            ...
            {
                if (MultipartRequestUtil.HasFileContentDisposition(contentDisposition))
                {
                ...
                else if (MultipartRequestUtil.HasFormDataContentDisposition(contentDisposition))
                {
        ...
        public async Task<IActionResult> UploadPhysical()
        {
            ...
            var boundary = MultipartRequestUtil.GetBoundary(MediaTypeHeaderValue.Parse(Request.ContentType), _defaultFormOptions.MultipartBoundaryLengthLimit);
            var reader = new MultipartReader(boundary, HttpContext.Request.Body);
            var section = await reader.ReadNextSectionAsync();
            ...
            while (section != null)
            {
                ...
                if (hasContentDispositionHeader)
                {
                    ...
                    else
                    {
                        ...
                        var streamedFileContent = await FileUtil.ProcessStreamedFile(section, contentDisposition, ModelState, _permittedExtensions, _fileSizeLimit);
                        ...
                        using (var targetStream = System.IO.File.Create(Path.Combine(_targetFilePath, trustedFileNameForFileStorage)))
                        {
                            ...
                            _logger.LogInformation("Uploaded file '{TrustedFileNameForDisplay}' saved to '{TargetFilePath}' as {TrustedFileNameForFileStorage}", trustedFileNameForDisplay, _targetFilePath, trustedFileNameForFileStorage);
                        }
                    }
                }
